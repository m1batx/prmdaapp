...
class MeatCategoriesPage extends StatefulWidget{
  final int index;
  const MeatCategoriesPage({super.key, required this.index});
  
  @override
  State<MeatCategoriesPage> createState() => _MeatCategoriesPageState();
}

class _MeatCategoriesPageState extends State<MeatCategoriesPage> {
  int indexCategory=0;
  List<int> index = [];
  ...
  void initState() {
   ...
   Widget categories() {
    List list = ['Курица', 'Фалафель', 'Говядина', 'Креветки', 'Напитки'];
    return SizedBox(
      height: 70,
  ...
  Widget gridFood() {
    ...
    MeatType selectedCategory = MeatType.values[indexCategory];

    // Filter the food items based on the selected category
    List<Food> filteredFoods = Restraunt().menu.where((food) => food.meatType == selectedCategory).toList();
